...
def boozer_surface_residual(surface, iota, biotsavart):
    ...
    xsemiflat = x.reshape((x.size//3, 3))
    ...
    residual = B - (np.sum(B**2, axis=1)/G)[:, None] * tang
    ...
    dresidual_dc =  dB_dc \
        - (2/G) * np.sum(B[..., None]*dB_dc, axis=1) * tang[..., None] \
        - (np.sum(B**2, axis=1)/G)[..., None] * (dxphi_dc + iota * dxtheta_dc)
    dresidual_diota = xtheta[..., None]
    ...
    dresidual_diota_flattened = dresidual_diota.reshape((nphi*ntheta*3, dresidual_diota.shape[-1]))
