...
class BiotSavart():
    ...
    def compute_A(self, points, compute_derivatives=0):
        ...
        for l in range(len(self.coils)):
            ...
            dgamma_by_dcoeff = dgamma_by_dcoeffs[l]
            d2gamma_by_dphidcoeff = d2gamma_by_dphidcoeffs[l]
            num_coil_quadrature_points = gamma.shape[0]
            for i, point in enumerate(points):
                diff = point-gamma
                ddiff_by_dcoeff = -dgamma_by_dcoeff
                dist = np.linalg.norm(diff, axis = 1)
                ddist_by_dcoeff = np.sum(ddiff_by_dcoeff,axis=1) / dist[:,None]
                ...
                if compute_derivatives >= 2:
                    for j1 in range(3):
                        for j2 in range(3):
                            term1 = 3 * (diff[:, j1] * diff[:, j2]/dist**5)[:, None] 
                            if j1 == j2:
                                term2 = - (1./dist**3)[:, None] 
                            else:
                                term2 = 0
                            self._d3A_by_dXdXdcoilcurrents[l][i, j1, j2, :] = np.sum( (term1 + term2)*dgamma_by_dphi , axis=0)
